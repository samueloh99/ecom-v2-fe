...
const Form = ({ cliente }) => {
  ...
  const [nivel, setNivel] = useState(String(cliente.roles[0].id))
  ...
  const editClient = useMutation(
    async (newValuesObj: {
      ...
      try {
        const response = await api.put(`/perfil/editar`, newValuesObj)
    ...
    <Stack w="50%">
      <FormControl mt={5} mb={100}>
        ...
        <Stack>
          ...
          <RadioGroup onChange={setNivel} value={nivel}>
            <Stack direction="row">
