...
const Form = ({ cupom }) => {
  ...
  const [fields, setFields] = useState<Cupom>(cupom)
  ...
  useEffect(() => {
    setDatas({
      ...datas,
      data1: new Date(fields.data_1).toLocaleDateString('pt-BR', {
        day: '2-digit',
        month: '2-digit',
        year: 'numeric'
      }),
      data2: new Date(fields.data_2).toLocaleDateString('pt-BR', {
        day: '2-digit',
        month: '2-digit',
        year: 'numeric'
      })
    ...
    async (newValuesObj: {
      ...
      try {
        const response = await api.patch(
          `/cupons/editar/${fields.id}`,
          newValuesObj
        )

        toast({
          title: 'Sucesso ao alterar.',
          description: 'Cupom foi alterado com sucesso',
          status: 'success',
          duration: 5000,
          isClosable: true
        })
